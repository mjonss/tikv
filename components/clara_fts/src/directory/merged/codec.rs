...
/// Encodes a MergedFile based on source files. The source files must not
/// contain subdirectories. The layout of the merged file is as follows:
/// - 4 bytes: length of serialized meta, in LittleEndian
/// - N bytes: serialized meta (in protobuf)
/// - N bytes: the content of all files note: the offset of each file is aligned
///   to 8 bytes boundary
///
...
impl<W: io::Write + io::Seek, R: io::Read> MergedFileEncoder<W, R> {
    ...
    pub fn finalize(mut self) -> Result<W> {
        ...
        for (file_name, ..) in self.buffered_files.iter() {
        ...
        for (file_name, _, size) in self.buffered_files.iter_mut() {
            ...
            // `offsets` field is fixed64, so we can fill it later without affecting
            // serialized size.
            meta.offsets.push(0);
        }
        ...
        // TODO: Discover ways to serialize into file directly without allocating a new
        // buffer
...
/// Decodes a MergedFile from either an in-memory buffer or a mmaped file
/// buffer.
