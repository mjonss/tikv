...
/// Produce a MergedFile to a buffer or file over the files in a blank
/// `Directory`.
pub trait MergedFileFromDirectory {
    // Note: this is not a object-safe trait, it cannot be boxed.
    // This trait is mostly useful in tests to test against different
    // implementations.
...
///
/// After all access is done, a MergedFile could be produced over the files in
/// the directory by calling `merge_files` and the directory will be removed.
///
...
impl MergedFileFromMmapDirectory {
    pub fn new(dir_path: &Path) -> Result<Self> {
        if dir_path.exists() {
            // We expect an empty directory. Otherwise TrackedDirectory will not work
            // correctly.
            bail!(
...
impl MergedFileFromDirectory for MergedFileFromMmapDirectory {
    fn merge_files<W: io::Write + io::Seek>(mut self, writer: W) -> Result<W> {
        // We require callers to ensure all directory references are dropped before
        // merging files. Otherwise after we merging all files in the directory,
        // the directory may be still accessed by someone else, causing
        // problems.
        ...
        // It is user's fault if there are concurrent writes to the source directory
        // during the merge and we will not handle this case.
...
impl Drop for MergedFileFromMmapDirectory {
    fn drop(&mut self) {
        // Note: self.directory may be none, or not. We simply drop it before removing
        // the directory so that possible usages are dropped.
        let d = self.tracked_directory.take();
        drop(d);
...
impl MergedFileFromDirectory for MergedFileFromRamDirectory {
        ...
        W: io::Write + io::Seek,
    {
        ...
        // Just a protection, in case of there is still some dangling reference to the
        // underlying directory. This could happen even when we checked
        // reference count is 1, because each sub-file is an Arc and can be kept
...
mod tests {
    ...
    fn test_mmap_alive_directory_reference() -> Result<()> {
        ...
        // Merging should be rejected because there is still a reference to the
        // directory.
