...
mod tests {
    ...
    fn test_prefix_column_handle_impl(columns: &[VarcharColumn]) {
        ...
        let key = table::encode_common_handle_row_key(TABLE_ID, &handle);
        ...
        let mut key_range = KeyRange::default();
        let begin = table::encode_common_handle_row_key(TABLE_ID - 1, &handle);
        let end = table::encode_common_handle_row_key(TABLE_ID + 1, &handle);
