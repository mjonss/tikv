...
impl<S: Storage> TableTask<S> {
    ...
    where
        F: KvFormat,
    {
        let mut table_scan = tipb::TableScan::default();
        ...
        BatchTableScanExecutor::new(self.storage, cfg, table_scan, self.key_ranges, None)
    }
}
    ...
    Handle: RowHandle,
{
    ...
    async fn next(&mut self) -> Option<TableTask<Self::Storage>> {
        ...
        if let Some(index_region_id) = self.accessor.get_original_region_id() {
            tikv_util::set_current_region(index_region_id);
        }
        None
    }
